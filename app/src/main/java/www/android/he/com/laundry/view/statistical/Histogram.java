...
public class Histogram extends Chart {
    ...
    private void initStyle(Context context, AttributeSet attrs) {
        ...
        rect_text_color = types.getColor(R.styleable.zqxchart_histogram_rectTextColor, Color.WHITE);
...
//            String ydata = formater.format(yData[i]);
            ...
            //画矩形上文字
            canvas.drawText(yData[i]+"km",left+10,textY,textPaint);
            //画每一个矩形
//            canvas.drawRect(left,top,right,oY,histogramPaint);

            RectF rectF = new RectF(left,top,right,oY);
            //画每一个圆形矩形
            canvas.drawRoundRect(rectF,50,50,histogramPaint);
        }
    }
    /*
     * 设置数据
